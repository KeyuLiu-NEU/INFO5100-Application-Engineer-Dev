...
public class SystemAdminWorkAreaJPanel extends javax.swing.JPanel {
    ...
    public void populateTree(){
        ...
        for(int i=0;i<ecosystem.getCustomerDirectory().getCustomerList().size();i++){
            Customer customer = ecosystem.getCustomerDirectory().getCustomerList().get(i);
            CustomerNode = new DefaultMutableTreeNode(customer.getName());
            networks.insert(CustomerNode, i);
        }
        
        for(int i=0;i<ecosystem.getRestaurantDirectory().getRestaurantList().size();i++){
            Restaurant restaurant = ecosystem.getRestaurantDirectory().getRestaurantList().get(i);
            RestaurantNode = new DefaultMutableTreeNode(restaurant.getName());
            networks.insert(RestaurantNode, i);           
        }
        
        for(int i=0;i<ecosystem.getDeliveryManDirectory().getDeliveryManList().size();i++){
            DeliveryMan delivertMan = ecosystem.getDeliveryManDirectory().getDeliveryManList().get(i);
            DeliveryManNode = new DefaultMutableTreeNode(delivertMan.getName());
            networks.insert(DeliveryManNode, i);           
        }
    ...
    private void btnManageNetworkActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnManageNetworkActionPerformed
            ManageCustomerJPanel panel = new ManageCustomerJPanel(userProcessContainer,ecosystem);
    ...
    private void btnManageAdminActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnManageAdminActionPerformed
            ManageDeliveryManJPanel panel = new ManageDeliveryManJPanel(userProcessContainer,ecosystem);
            userProcessContainer.add("ManageDeliveryManJPanel", panel);
            CardLayout layout = (CardLayout) userProcessContainer.getLayout();
            layout.next(userProcessContainer);
